...
export default function StatsOverlay({ v }) {
  return (
    <OverlayShell title="Statistika" onClose={v.close}>
      <ScrollView contentContainerStyle={{ paddingTop: 12, paddingHorizontal: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <View style={[st.card, { padding: 20 }]}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 16 }}>
            <Text style={{ fontFamily: F.bold, fontSize: 15, color: C.cream }}>Namoz — bu hafta</Text>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 8, height: 120 }}>
            {v.prayerWeek.map((d, i) => (
              <View key={i} style={{ flex: 1, alignItems: 'center', gap: 7, height: '100%', justifyContent: 'flex-end' }}>
                ...
                <Text style={{ fontFamily: F.regular, fontSize: 11, color: C.sageFaint }}>{d.d}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={[st.card, { padding: 18 }]}>
          <Text style={{ fontFamily: F.bold, fontSize: 15, color: C.cream, marginBottom: 14 }}>Odat streaklari</Text>
          <View style={{ gap: 12 }}>
            {v.habits.map((h, i) => (
              <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                <Text style={{ flex: 1, fontFamily: F.regular, fontSize: 14, color: '#C6D4CC' }}>{h.name}</Text>
                <Text style={{ fontFamily: F.extrabold, fontSize: 13, color: C.gold }}>{h.streak} kun</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={[st.card, { padding: 18, marginBottom: 0 }]}>
          <Text style={{ fontFamily: F.bold, fontSize: 15, color: C.cream, marginBottom: 14 }}>{v.roleLabel} reytingi</Text>
          <View style={{ gap: 14 }}>
            {v.leaderboard.map(m => (
