...
export default function MemberOverlay({ v }) {
  ...
  return (
    <OverlayShell title="A'zo profili" onClose={v.close}>
      <ScrollView contentContainerStyle={{ paddingTop: 12, paddingHorizontal: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 20 }}>
          ...
          <View>
            ...
            <Text style={{ fontFamily: F.regular, fontSize: 14, color: C.sage, marginTop: 2 }}>{m.label}</Text>
          </View>
        </View>

        <View style={st.progressCard}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
            <Text style={{ fontFamily: F.regular, fontSize: 14, color: C.sage }}>Bajarilgan vazifalar</Text>
        ...
        <PrimaryBtn label="+ Bu a'zoga vazifa yoki eslatma" onPress={m.onAssign} style={{ marginBottom: 22, paddingVertical: 14 }} />

        <SectionTitle style={{ fontSize: 18, marginHorizontal: 2, marginBottom: 12 }}>Vazifalari</SectionTitle>
        <View style={{ gap: 11 }}>
          {m.tasks.map(t => (
            <TouchableOpacity key={t.id} onPress={t.onOpen} activeOpacity={0.85} style={st.taskRow}>
              <View style={{ flex: 1, minWidth: 0 }}>
                ...
                <Text style={{ fontFamily: F.regular, fontSize: 12, color: C.sage, marginTop: 2 }}>{t.cat} · {t.due}</Text>
              </View>
