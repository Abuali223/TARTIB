...
function Row({ active, onPress, iconBg, icon, title, sub, showCheck = true }) {
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.85}
      style={[st.row, active ? st.rowActive : st.rowIdle]}>
        ...
        <LinearGradient colors={['#102c22', '#0a1f18']} style={st.sheet}>
          ...
          {!creating ? (
            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 460 }}>
              ...
              {/* Shaxsiy */}
              <Row
                active={v.shaxsiyActive}
                onPress={v.onSelectShaxsiy}
                iconBg="rgba(217,179,106,0.14)"
                icon={<PersonIcon />}
                title="Shaxsiy"
                sub="O'zim uchun ibodat va odatlar"
              />

              {/* Foydalanuvchi makonlari */}
              {v.myWorkspaces.map(w => {
                ...
                return (
                  <Row
                    ...
                    onPress={w.onSelect}
                    iconBg={ic.bg}
                    icon={<Icon color={ic.color} />}
                    title={w.name}
                    sub={`${w.typeLabel} · ${w.role}`}
                  />
                );
              })}

              {/* Yangi makon */}
              <TouchableOpacity onPress={() => setCreating(true)} activeOpacity={0.85} style={st.createRow}>
            ...
            <View>
              ...
              <View style={{ flexDirection: 'row', gap: 10, marginBottom: 18 }}>
                {v.wsTypeChips.map((t, i) => {
                  const ic = TYPE_ICON[t.k];
                  const Icon = ic.Icon;
                  return (
                    <TouchableOpacity key={i} onPress={t.onPick} activeOpacity={0.85}
                      style={[st.typeChip, t.active && { borderColor: ic.color, backgroundColor: ic.color + '18' }]}>
              ...
              <TextInput
                value={v.wsDraft.name}
                onChangeText={v.onWsName}
              ...
              <TouchableOpacity onPress={() => setCreating(false)} activeOpacity={0.8} style={{ paddingVertical: 12, alignItems: 'center' }}>
...
const st = StyleSheet.create({
  ...
  createRow: {
    flexDirection: 'row', alignItems: 'center', gap: 14, width: '100%',
    paddingVertical: 15, paddingHorizontal: 16, borderRadius: 16, marginBottom: 4,
    borderWidth: 1, borderColor: 'rgba(217,179,106,0.25)', borderStyle: 'dashed',
  },
  ...
  input: {
    ...
    borderWidth: 1, borderColor: 'rgba(217,179,106,0.18)', color: C.cream, fontFamily: F.medium, fontSize: 15, marginBottom: 20,
  },
